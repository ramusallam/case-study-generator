...
export default function TeacherStudio() {
  ...
  const [libraryDiscipline, setLibraryDiscipline] = useState<string>('all');
  const presentRef = useRef<HTMLDivElement>(null);

  const activeCase = data?.cases?.[activeIndex] ?? null;

  // Load saved cases on mount and when switching to library
  useEffect(() => {
    if (appView === 'library') {
  ...
  useEffect(() => {
    ...
    function onKey(e: KeyboardEvent) {
      if (e.key === 'ArrowRight' || e.key === ' ') { e.preventDefault(); setRevealIndex((v) => Math.min(REVEAL_STAGES.length - 1, v + 1)); }
      else if (e.key === 'ArrowLeft') { e.preventDefault(); setRevealIndex((v) => Math.max(0, v - 1)); }
  ...
  function selectDiscipline(d: Discipline) {
    ...
    setForm((prev) => ({ ...prev, discipline: d, course: d === 'custom' ? prev.course : preset.label }));
  }

  const handleGenerate = useCallback(async (overrides?: Partial<GenerationForm>) => {
    if (!navigator.onLine) { setError('You appear to be offline. Check your connection and try again.'); return; }
    setLoading(true);
    setError(null);
    try {
  ...
  function handleRefine(action: string) {
    ...
    handleGenerate({ count: 1, refinement: { action, sourceCase: activeCase, excludeDiagnosis: action === 'different-diagnosis' ? activeCase.correctDiagnosis : undefined } });
  }
  ...
  function updateDiff(index: number, field: keyof Differential, value: string) {
    setEditCase((p) => {
      if (!p) return p;
      const diffs = [...p.differentialDiagnoses];
      diffs[index] = { ...diffs[index], [field]: value };
      return { ...p, differentialDiagnoses: diffs };
    });
  }
  ...
  async function handleSaveCase(c: CaseStudy) {
    try {
      await saveCase(c, form.discipline, form.course);
      showToast('Case saved to library');
    } catch {
  ...
  async function handleToggleFavorite(id: string, fav: boolean) {
    ...
    setSavedCases((prev) => prev.map((c) => c.id === id ? { ...c, favorite: fav } : c));
  }
  ...
  function loadSavedCase(c: SavedCase) {
    setData({ cases: [c] });
    setActiveIndex(0);
    setMode('teacher');
    setAppView('studio');
  ...
  function toggleFullscreen() {
    ...
    else presentRef.current.requestFullscreen();
  }
  ...
  const filteredCases = useMemo(() => {
    ...
    if (libraryDiscipline !== 'all') cases = cases.filter((c) => c.discipline === libraryDiscipline);
    if (libraryFilter.trim()) {
      const q = libraryFilter.toLowerCase();
      cases = cases.filter((c) =>
        c.title.toLowerCase().includes(q) ||
        c.correctDiagnosis.toLowerCase().includes(q) ||
        c.course.toLowerCase().includes(q) ||
        (c.teacherNotes.coreConcepts ?? []).some((cc) => cc.toLowerCase().includes(q))
      );
    }
    return cases;
  }, [savedCases, libraryFilter, libraryDiscipline]);
        ...
        <div className="app-header-inner">
          ...
          <div className="header-right">
            ...
            <div className="mode-tabs">
              <button className={`mode-tab ${appView === 'studio' ? 'mode-tab-active' : ''}`} onClick={() => setAppView('studio')}>Studio</button>
              <button className={`mode-tab ${appView === 'library' ? 'mode-tab-active' : ''}`} onClick={() => setAppView('library')}>
                Library{savedCases.length > 0 ? ` (${savedCases.length})` : ''}
              </button>
            </div>
      ...
      {appView === 'library' && (
        <div className="library-container">
          <div className="library-header">
            <div>
              <h2 className="library-title">Case Library</h2>
              <p className="library-subtitle">{savedCases.length} saved case{savedCases.length !== 1 ? 's' : ''}</p>
            </div>
            <div className="library-controls">
              ...
              />
              <select className="field-select library-filter" value={libraryDiscipline} onChange={(e) => setLibraryDiscipline(e.target.value)}>
                <option value="all">All disciplines</option>
                {DISCIPLINE_KEYS.map((d) => <option key={d} value={d}>{DISCIPLINES[d].label}</option>)}
              </select>
            </div>
          </div>

          {libraryLoading ? (
            <div className="empty-state"><div className="spinner-lg" /><h2>Loading library...</h2></div>
          ) : filteredCases.length === 0 ? (
            <div className="empty-state">
              <div className="empty-icon">&#128218;</div>
              <h2>{savedCases.length === 0 ? 'No saved cases yet' : 'No matching cases'}</h2>
              <p>{savedCases.length === 0
                ? 'Generate cases in the Studio, then save the ones you like. They\'ll appear here for reuse anytime.'
              ...
              {filteredCases.map((c) => (
                <div className="library-card" key={c.id}>
                  <div className="library-card-header">
                    <h3 className="library-card-title">{c.title}</h3>
                    <button
                      className={`btn-icon ${c.favorite ? 'btn-icon-active' : ''}`}
                      onClick={() => handleToggleFavorite(c.id, !c.favorite)}
                      aria-label={c.favorite ? 'Remove from favorites' : 'Add to favorites'}
                      title={c.favorite ? 'Favorited' : 'Favorite'}
                    >
                      {c.favorite ? '\u2605' : '\u2606'}
                    </button>
                  </div>
                  <p className="library-card-summary">{c.summary.slice(0, 140)}{c.summary.length > 140 ? '...' : ''}</p>
                  <div className="library-card-meta">
                    <span className="library-tag">{c.discipline}</span>
                    <span className="library-tag">{c.correctDiagnosis}</span>
                    <span className="library-date">{new Date(c.savedAt).toLocaleDateString()}</span>
                  </div>
                  <div className="library-card-actions">
                    <button className="btn btn-sm btn-primary" onClick={() => loadSavedCase(c)}>Load</button>
                    <button className="btn btn-sm btn-danger" onClick={() => handleDeleteCase(c.id)}>Delete</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="form-section">
              ...
              <div className="field-stack">
                ...
                <div>
                  ...
                  <input className="field-input" value={form.topicTargets} onChange={(e) => updateForm('topicTargets', e.target.value)} placeholder="e.g., Action potentials, myelin, neuron signaling" maxLength={5000} />
                </div>
                <div>
                  <label className="field-label">What should students uncover?</label>
                  <textarea className="field-textarea" value={form.scientificTunnel} onChange={(e) => updateForm('scientificTunnel', e.target.value)} placeholder="The scientific concepts the case should tunnel toward..." maxLength={5000} />
                </div>
                <div>
                  <label className="field-label">Preferred diagnosis <span className="field-optional">optional</span></label>
                  <input className="field-input" value={form.preferredDiagnosis || ''} onChange={(e) => updateForm('preferredDiagnosis', e.target.value)} placeholder="e.g., Multiple sclerosis" maxLength={500} />
                </div>
                <div>
                  <label className="field-label">Student task <span className="field-optional">optional</span></label>
                  <textarea className="field-textarea field-textarea-sm" value={form.studentTask || ''} onChange={(e) => updateForm('studentTask', e.target.value)} placeholder="Default: Determine the most likely diagnosis with 2-3 differentials." maxLength={2000} />
                </div>
              </div>
            </div>
            ...
            <div className="form-section">
              ...
              <div className="field-stack">
                ...
                <div>
                  ...
                  <input className="field-input" value={form.focusNotes || ''} onChange={(e) => updateForm('focusNotes', e.target.value)} placeholder="Authenticity, misconceptions, phenotype..." maxLength={2000} />
                </div>
                <div>
                  <label className="field-label">Guardrails <span className="field-optional">optional</span></label>
                  <input className="field-input" value={form.constraints || ''} onChange={(e) => updateForm('constraints', e.target.value)} placeholder="No rare jargon, keep diagnosis non-obvious..." maxLength={2000} />
                </div>
              </div>
            </div>
          ...
          <main className="results-area">
            ...
            {!data && !loading && (
              <div className="empty-state">
                ...
                <p>Choose a discipline, enter your topic and what you want students to uncover, then generate. Cases are saved to your library for reuse.</p>
              </div>
            )}
              ...
              <>
                ...
                {mode === 'teacher' && activeCase && (
                  <div className="quick-actions">
                    ...
                    {[
                      ['generate-more', 'More like this'],
                      ['tighten-tunnel', 'Tighten tunnel'],
                      ['more-ambiguous', 'More ambiguous'],
                      ['more-realistic', 'More realistic'],
                      ['more-student-friendly', 'Simpler'],
                      ['different-diagnosis', 'Different diagnosis'],
                    ].map(([action, label]) => (
                      <button key={action} className="btn btn-sm" onClick={() => handleRefine(action)} disabled={loading}>{label}</button>
                    ))}
                  </div>
                )}
                  ...
                  <div className="case-view">
                    <div className="case-header-card">
                      <div className="case-header-top">
                        ...
                        <button className="btn btn-sm btn-primary" onClick={() => handleSaveCase(activeCase)} aria-label="Save to library">Save to library</button>
                      </div>
                      ...
                      {REVEAL_STAGES.map((stage) => {
                        ...
                        return (
                          <div className="reveal-card" key={stage.key}>
                            ...
                            <ul className="reveal-list">{items.map((item, i) => <li key={i}>{item}</li>)}</ul>
                          </div>
                        );
                      })}
                    </div>
                    ...
                    <div className="teacher-grid">
                      <div className="teacher-card"><h3 className="section-title">Correct Diagnosis</h3><p className="teacher-highlight">{activeCase.correctDiagnosis}</p></div>
                      <div className="teacher-card"><h3 className="section-title">Content Tunnel</h3><p>{activeCase.teacherNotes.contentTunnel}</p></div>
                    </div>

                    <div className="teacher-grid">
                      <div className="teacher-card"><h3 className="section-title">Clues Toward Diagnosis</h3><ul className="tag-list">{(activeCase.diagnosticClues ?? []).map((c, i) => <li key={i}>{c}</li>)}</ul></div>
                      <div className="teacher-card"><h3 className="section-title">What Keeps the Differential Open</h3><ul className="tag-list">{(activeCase.differentialClues ?? []).map((c, i) => <li key={i}>{c}</li>)}</ul></div>
                    </div>

                    <div className="teacher-grid">
                      <div className="teacher-card">
                        <h3 className="section-title">Core Concepts</h3>
                        <div className="concept-tags">{(activeCase.teacherNotes.coreConcepts ?? []).map((c, i) => <span className="concept-tag" key={i}>{c}</span>)}</div>
                      </div>
                      <div className="teacher-card">
                    ...
                    {(activeCase.suggestedNextTests ?? []).length > 0 && (
                      <div className="teacher-card"><h3 className="section-title">Suggested Next Tests</h3><ul className="tag-list">{activeCase.suggestedNextTests.map((t, i) => <li key={i}>{t}</li>)}</ul></div>
                    )}

                    <div className="teacher-card"><h3 className="section-title">Why This Case Works</h3><p>{activeCase.teacherNotes.whyThisCaseWorks}</p></div>
                ...
                {activeCase && editCase && mode === 'editor' && (
                  <div className="case-view">
                    <div className="editor-notice">Edit any field. Changes stay in this session until you save.</div>

                    <div className="edit-section"><label className="edit-label">Title</label><input className="field-input" value={editCase.title} onChange={(e) => updateEditField('title', e.target.value)} /></div>
                    <div className="edit-section"><label className="edit-label">Summary</label><textarea className="field-textarea" value={editCase.summary} onChange={(e) => updateEditField('summary', e.target.value)} /></div>
                    <div className="edit-section"><label className="edit-label">Student Task</label><textarea className="field-textarea field-textarea-sm" value={editCase.studentPrompt} onChange={(e) => updateEditField('studentPrompt', e.target.value)} /></div>

                    {REVEAL_STAGES.map((stage) => (
                      <div className="edit-section" key={stage.key}>
                        <label className="edit-label">{stage.label}</label>
                        <textarea className="field-textarea" value={(editCase.progressiveReveal[stage.key] ?? []).join('\n')} onChange={(e) => updateEditField(`progressiveReveal.${stage.key}`, e.target.value.split('\n').filter((l) => l.trim()))} />
                        <span className="edit-hint">One item per line</span>
                      </div>
                    ))}

                    <div className="edit-section"><label className="edit-label">Correct Diagnosis</label><input className="field-input" value={editCase.correctDiagnosis} onChange={(e) => updateEditField('correctDiagnosis', e.target.value)} /></div>
                    ...
                    {(editCase.differentialDiagnoses ?? []).map((d, i) => (
                      <div className="edit-section edit-diff" key={i}>
                        ...
                        <input className="field-input" value={d.diagnosis} placeholder="Diagnosis" onChange={(e) => updateDiff(i, 'diagnosis', e.target.value)} />
                        <input className="field-input" value={d.whyItFits} placeholder="Why it fits" onChange={(e) => updateDiff(i, 'whyItFits', e.target.value)} />
                        <input className="field-input" value={d.whyItFallsShort} placeholder="Why it falls short" onChange={(e) => updateDiff(i, 'whyItFallsShort', e.target.value)} />
                        <button className="btn btn-sm btn-danger" onClick={() => setEditCase((p) => p ? { ...p, differentialDiagnoses: p.differentialDiagnoses.filter((_, idx) => idx !== i) } : p)}>Remove</button>
                      </div>
                    ))}
                    ...
                    <div className="edit-section">
                      ...
                      <div className="field-stack">
                        <div><span className="edit-hint">Content tunnel</span><textarea className="field-textarea field-textarea-sm" value={editCase.teacherNotes.contentTunnel} onChange={(e) => updateEditField('teacherNotes.contentTunnel', e.target.value)} /></div>
                        <div><span className="edit-hint">Core concepts — one per line</span><textarea className="field-textarea field-textarea-sm" value={(editCase.teacherNotes.coreConcepts ?? []).join('\n')} onChange={(e) => updateEditField('teacherNotes.coreConcepts', e.target.value.split('\n').filter((l) => l.trim()))} /></div>
                        <div><span className="edit-hint">Misconceptions to watch — one per line</span><textarea className="field-textarea field-textarea-sm" value={(editCase.teacherNotes.misconceptionsToWatch ?? []).join('\n')} onChange={(e) => updateEditField('teacherNotes.misconceptionsToWatch', e.target.value.split('\n').filter((l) => l.trim()))} /></div>
                        <div><span className="edit-hint">Why this case works</span><textarea className="field-textarea field-textarea-sm" value={editCase.teacherNotes.whyThisCaseWorks} onChange={(e) => updateEditField('teacherNotes.whyThisCaseWorks', e.target.value)} /></div>
                      </div>
                    </div>
                  ...
                  <div className="presentation-shell" ref={presentRef}>
                    <div className="presentation-header">
                      ...
                      <div className="presentation-controls">
                        ...
                        <button className="btn btn-present" onClick={() => setRevealIndex((v) => Math.max(0, v - 1))} disabled={revealIndex === 0} aria-label="Previous stage">Back</button>
                        <button className="btn btn-present btn-present-primary" onClick={() => setRevealIndex((v) => Math.min(REVEAL_STAGES.length - 1, v + 1))} disabled={revealIndex === REVEAL_STAGES.length - 1} aria-label="Next stage">Reveal Next</button>
                        <button className="btn btn-present" onClick={toggleFullscreen} aria-label="Toggle fullscreen">Fullscreen</button>
                    ...
                    <div className="share-layout">
                      ...
                      <div className="share-details">
                        <div className="share-info">
                          <h3>Student Link</h3>
                          <p>Students see the case and progressive reveal only. No teacher notes, diagnoses, or answer key.</p>
